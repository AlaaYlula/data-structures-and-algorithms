package graph.depth.first.structure;

import graph.depth.first.data.Edge;
...
public class Graph<T> {
    ...
    //Depth First
    public Set<T> DepthFirst(Vertex<T> root){
        Set<T> visited = new LinkedHashSet<>();
        ...
        while (!stack.isEmpty()) {
            ...
            Set<Vertex<T>> neighbors = getNeighbors(vertex);
            List<Vertex<T>> listNeighbors = new ArrayList<>();
            listNeighbors.addAll(neighbors);
            System.out.println("Neighbor "+vertex);
            System.out.println("before reverse => "+listNeighbors);
            // https://www.youtube.com/watch?v=x6iO0ZH9h7Q
            Collections.reverse(listNeighbors);
            System.out.println("After reverse => "+listNeighbors);
            System.out.println("///////////");
            if (neighbors != null) {
                for (Vertex<T> v : listNeighbors) {
                    if (!visited.contains(v.getData())) {
